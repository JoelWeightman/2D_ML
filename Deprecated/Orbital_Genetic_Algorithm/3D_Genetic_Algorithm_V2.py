...
def velocity_distance_2d(pop, dt, acc, t_steps, init_conds, const_conds):
    ...
    r = np.zeros((np.shape(pop['actions'])[0],np.shape(pop['actions'])[1],1))
    theta = np.zeros((np.shape(pop['actions'])[0],np.shape(pop['actions'])[1],1))
    
    r_dot = np.zeros((np.shape(pop['actions'])[0],np.shape(pop['actions'])[1],1))
    theta_dot = np.zeros((np.shape(pop['actions'])[0],np.shape(pop['actions'])[1],1))
    
    r[:,0,0] = init_conds[0]
    theta[:,0,0] = init_conds[2]
    
    r_dot[:,0,0] = init_conds[1]
    theta_dot[:,0,0] = init_conds[3]
      
    for i in range(np.shape(pop['actions'])[1]-1):
        r_dot_dot = pop['actions'][:,i,0]*np.cos(pop['actions'][:,i,1]) + const_conds[0]
        theta_dot_dot = pop['actions'][:,i,0]*np.sin(pop['actions'][:,i,1])/r[:,i,0]
        
        r_dot[:,i+1,0] = r_dot_dot*dt + r_dot[:,i,0]
        theta_dot[:,i+1,0] = theta_dot_dot*dt + theta_dot[:,i,0]
        
        r[:,i+1,0] = r_dot[:,i,0]*dt + r[:,i,0]
        theta[:,i+1,0] = theta_dot[:,i,0]*dt + theta[:,i,0]

    return r_dot[:,-1],theta_dot[:,-1], r[:,-1], theta[:,-1]
...
def velocity_distance_3d(pop, dt, acc, t_steps, init_conds, const_conds):
    ...
    r = np.zeros((np.shape(pop['actions'])[0],np.shape(pop['actions'])[1],1))
    theta = np.zeros((np.shape(pop['actions'])[0],np.shape(pop['actions'])[1],1))
    phi = np.zeros((np.shape(pop['actions'])[0],np.shape(pop['actions'])[1],1))
    
    r_dot = np.zeros((np.shape(pop['actions'])[0],np.shape(pop['actions'])[1],1))
    theta_dot = np.zeros((np.shape(pop['actions'])[0],np.shape(pop['actions'])[1],1))
    phi_dot = np.zeros((np.shape(pop['actions'])[0],np.shape(pop['actions'])[1],1))
    
    r[:,0,0] = init_conds[0]
    theta[:,0,0] = init_conds[2]
    phi[:,0,0] = init_conds[4]
    
    r_dot[:,0,0] = init_conds[1]
    theta_dot[:,0,0] = init_conds[3]
    phi_dot[:,0,0] = init_conds[5] 
    
    for i in range(np.shape(pop['actions'])[1]):
        r_dot_dot = pop['actions'][:,i,0]*np.cos(pop['actions'][:,i,1])*np.sin(pop['actions'][:,i,2]) + const_conds[0]
        theta_dot_dot = pop['actions'][:,i,0]*np.sin(pop['actions'][:,i,1])*np.sin(pop['actions'][:,i,2])
        phi_dot_dot = pop['actions'][:,i,0]*np.cos(pop['actions'][:,i,2])
        
        r_dot[:,i+1,0] = r_dot_dot*dt + r_dot[:,i,0]
        theta_dot[:,i+1,0] = theta_dot_dot*dt + theta_dot[:,i,0]
        phi_dot[:,i+1,0] = phi_dot_dot*dt + phi_dot[:,i,0]
        
        r[:,i+1,0] = r_dot[:,i,0]*dt + r[:,i,0]
        theta[:,i+1,0] = theta_dot[:,i,0]*dt + theta[:,i,0]
        phi[:,i+1,0] = phi_dot[:,i,0]*dt + phi[:,i,0]

    return r_dot[:,-1],theta_dot[:,-1], phi_dot[:,-1], r[:,-1], theta[:,-1], phi[:,-1]
...
#    
#    d_x[:,0,0] = init_conds[0]*np.sin(init_conds[2])*np.cos(init_conds[4])
#    d_y[:,0,0] = init_conds[0]*np.sin(init_conds[2])*np.sin(init_conds[4])
#    d_z[:,0,0] = init_conds[0]*np.cos(init_conds[2])
#    
#    v_x[:,0,0] = (init_conds[1]*np.sin(init_conds[2])*np.cos(init_conds[4]) + 
#        init_conds[0]*init_conds[3]*np.cos(init_conds[2])*np.cos(init_conds[4]) - 
#        init_conds[0]*init_conds[5]*np.sin(init_conds[2])*np.sin(init_conds[4]))
...
#    v_z[:,0,0] = (init_conds[1]*np.cos(init_conds[2]) - 
#        init_conds[0]*init_conds[3]*np.sin(init_conds[2])
#    
#    r_cur = init_conds[0]
#    theta_cur = init_conds[2]
#    phi_cur = init_conds[4]
#    
#    for i in range(1,np.shape(pop['actions'])[1]):
#        
#        
#        
#        
