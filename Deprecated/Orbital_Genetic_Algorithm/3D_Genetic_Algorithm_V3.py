...
def plot_actions(pop, acc, t_steps, dt, dimensions, init_conds, const_conds):
    ...
    for i in range(np.shape(pop['actions'])[1]-1):
        ...
        a_y = acc*pop['actions'][:,i,0]*np.sin(pop['actions'][:,i,1]) + const_conds[0]*np.sin(theta_cur[:,0])
    ...
    r_cur = np.sqrt(d_x[best_index]**2+d_y[best_index]**2)
    theta_cur = np.arctan2(d_y[best_index],d_x[best_index])
    
    r_dot = v_x[best_index]*np.cos(theta_cur)-v_y[best_index]*np.sin(theta_cur)#(d_x[best_index]*v_x[best_index] + d_y[best_index]*v_y[best_index])/r_cur
    theta_dot = (v_x[best_index]*np.sin(theta_cur)+v_y[best_index]*np.cos(theta_cur))/r_cur#(d_x[best_index]*v_y[best_index] - d_y[best_index]*v_x[best_index])/(d_x[best_index]**2)*np.cos(theta_cur)**2
...
#    plt.draw()
    ...
    fig = plt.figure(4)
    plt.clf()
    c = plt.scatter(d_x[best_index], d_y[best_index], c = 'black')
    plt.axis('equal')
    plt.pause(0.001)
    plt.draw()
...
def velocity_distance_2d(pop, dt, acc, t_steps, init_conds, const_conds):
    ...
    for i in range(np.shape(pop['actions'])[1]-1):
        a_x = acc*pop['actions'][:,i,0]*np.cos(pop['actions'][:,i,1]) + const_conds[0]*(R_E/r_cur[:,0])**2*np.cos(theta_cur[:,0])
        a_y = acc*pop['actions'][:,i,0]*np.sin(pop['actions'][:,i,1]) + const_conds[0]*(R_E/r_cur[:,0])**2*np.sin(theta_cur[:,0])
    ...
    r_dot = v_x[:,-1]*np.cos(theta_cur) - v_y[:,-1]*np.sin(theta_cur)#(d_x[best_index]*v_x[best_index] + d_y[best_index]*v_y[best_index])/r_cur
    theta_dot = (v_x[:,-1]*np.sin(theta_cur) + v_y[:,-1]*np.cos(theta_cur))/r_cur#(d_x[best_index]*v_y[best_index] - d_y[best_index]*v_x[best_index])/(d_x[best_index]**2)*np.cos(theta_cur)**2
