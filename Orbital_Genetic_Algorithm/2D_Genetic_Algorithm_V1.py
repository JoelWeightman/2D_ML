...
def initialise_population(dimensions, t_steps, pop_size):
    
    pop = dict({'actions': np.zeros((pop_size,t_steps,dimensions)), 'results':np.zeros((pop_size,dimensions*2)), 'score':np.zeros((pop_size))})

    pop['actions'] = initial_values(dimensions,pop['actions'])

    return pop

def initial_values(dimensions, actions):
    
    if dimensions == 1:
        actions[:,:,0] = np.random.randint(-1, 2, size = np.shape(actions[:,:,0]))
    elif dimensions == 2:
        actions[:,:,0] = np.random.randint(-1, 2, size = np.shape(actions[:,:,0]))
        actions[:,:,1] = np.random.randint(-1, 2, size = np.shape(actions[:,:,0]))
...
def get_generation_nums(t_steps, pop_size, generation_weights):
    ...
    mutated_gene_max = np.round(generation_weights[-1]*t_steps).astype('int64')
...
def pop_selection(pop, generation_nums, dimensions, I):
    ...
    mutated_pop = np.setdiff1d(np.arange(np.size(sorted_pop)),np.concatenate((elite_pop,selected_pop,lucky_pop)))
    mutated_pop = np.random.choice(mutated_pop,size = generation_nums[3], replace = False)

    actions = pop['actions'][np.concatenate((elite_pop,lucky_pop)).astype('int64')]

    pop = generate_children(pop, actions, generation_nums, selected_pop, mutated_pop, dimensions, I)
    
    return pop, np.array([np.array(pop['score'][sorted_pop[0]]),pop['results'][sorted_pop[0]],sorted_pop[0]])
    
def generate_children(pop, actions, generation_nums, selected_pop, mutated_pop, dimensions, I):
    ...
    for i, mut_locs in enumerate(indices):
        ...
        mutated_actions[i,mut_locs] = np.random.randint(-1, 2, size = (mut_num,dimensions))

#        if dimensions == 2:
#            mutated_actions[i,mut_locs,1] = np.random.randint(-1, 2, size = mut_num)
...
def run_GA_result(inputs,dimensions, goal, GA_parameters, parameters, generation_weights, samples):
    ...
    pop, gen_count_stats = run_GA(dimensions, goal, GA_parameters, parameters, inputs, generation_weights, samples)
...
def run_GA_generation(inputs,dimensions, goal, GA_parameters, parameters, result_weights, samples):
    ...
    pop, gen_count_stats = run_GA(dimensions, goal, GA_parameters, parameters, result_weights, inputs, samples)
    
    return gen_count_stats[0]

def run_GA(dimensions, goal, GA_parameters, parameters, result_weights, generation_weights, samples):
    ...
    for KK in range(samples):  # Count for samples
        ...
        pop = initialise_population(dimensions, t_steps, pop_size)
        ...
        for I in range(max_gen):
            ...
            pop, best_performance = pop_selection(pop, generation_nums, dimensions, I)

            if np.mod(I,max_gen/100) == 0 and I != 0:
...
def set_parameters(dimensions, goal):
    ...
    elite_weight = 0.44
    lucky_weight = 0.05
    mutation_weight = 0.55
    non_children_chance = (elite_weight + lucky_weight + mutation_weight)
    children_weight = 0.3 #np.ceil(non_children_chance) - non_children_chance
...
#    generation_weights = generation_weights/np.sum(generation_weights)
    ...
    if optimize_result_weights == True:
        ...
        pop, gen_count_stats  = run_GA(dimensions, goal, GA_parameters, parameters, result_weights, generation_weights, samples)
