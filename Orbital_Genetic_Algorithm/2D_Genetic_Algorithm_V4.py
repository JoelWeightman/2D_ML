...
def get_acceleration(dimensions, goal, dt, t_steps, ideal_time_perc, mass, thrust):
    ...
    if False:
        u_acc = thrust/mass
        u_max = u_acc*(dt*t_steps*ideal_time_perc/2)
...
def initialise_population(dimensions, t_steps, pop_size, steps, deg_steps):
    ...
    pop['actions'] = initialise_values(dimensions,pop['actions'],steps, deg_steps)

    return pop

def initialise_values(dimensions, actions, steps, deg_steps):
#    steps = 4
    if dimensions == 1:
...
def velocity_distance_1d(pop, dt, acc, t_steps, init_conds, const_conds):
    ...
    v[:,1:t_steps,0] = np.cumsum(np.multiply(pop['actions'][:,:-1],(acc+const_conds[0])*dt), axis = 1)[:,:,0]+init_conds[1]
    
    v[:,0,0] = init_conds[1]
    
    d = np.trapz(v, dx = dt, axis = 1)+init_conds[0]
...
def initial_conditions(dimensions):
    ...
    return np.array([x_init, u_init, y_init, v_init])
...
def constant_conditions(dimensions):
    ...
    return np.array([u_acc_const, v_acc_const])
...
def set_parameters(dimensions, goal):
    ...
    vel_max, acc = get_acceleration(dimensions, goal, dt, t_steps, ideal_time_perc, mass, thrust)
