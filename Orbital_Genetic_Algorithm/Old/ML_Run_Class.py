...
def __init__(self, A_l, A_v, A_f, A_t, elite_perc, luck_perc, mutation_perc, mutation_chance, pop_size):
    
    setattr(self, 'A_l', A_l)
    setattr(self, 'A_v', A_v)
    setattr(self, 'A_f', A_f)
    setattr(self, 'A_t', A_t)
    setattr(self, 'elite_perc', elite_perc)
    setattr(self, 'luck_perc', luck_perc)
    setattr(self, 'mutation_perc', mutation_perc)
    setattr(self, 'mutation_chance', mutation_chance)
    setattr(self, 'pop_size', pop_size)
    
    
    self.gen_count = 10000
    self.t_steps = 50
...
#                print('Gen %1d Performance %3.3f, Distance = %3.1f, Velocity = %3.3f' % (I, best_performance[0], best_performance[1][0], best_performance[1][1]))
    ...
    gen_count_stats[2] = np.var(gen_count[1,:],1)
